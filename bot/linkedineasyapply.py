...
class LinkedinEasyApply:
    ...
    def apply_jobs(self, location):
        no_jobs_text = ""
        try:
            no_jobs_element = WebElementWrapper(
                self.browser, By.CLASS_NAME, 'jobs-search-two-pane__no-results-banner--expand').find()
            if no_jobs_element:
        ...
        try:
            job_results = WebElementWrapper(
                self.browser, By.CLASS_NAME, "jobs-search-results-list").find()
            if job_results:
                self.scroll_slow(job_results)
                self.scroll_slow(job_results, step=300, reverse=True)

                job_list = WebElementWrapper(
                    self.browser, By.CLASS_NAME, 'scaffold-layout__list-container').find().find_elements(
        ...
        for job_tile in job_list:
            ...
            try:
                job_title_element = WebElementWrapper(
                    job_tile, By.CLASS_NAME, 'job-card-list__title').find()
                if job_title_element:
                    job_title = job_title_element.text
                    link = job_title_element.get_attribute(
                        'href').split('?')[0]
            except:
                pass
            try:
                company_element = WebElementWrapper(
                    job_tile, By.CLASS_NAME, 'job-card-container__company-name').find()
                if company_element:
                    company = company_element.text
            except:
                pass
            try:
                hiring_line = WebElementWrapper(
                    job_tile, By.XPATH, '//span[contains(.,\' is hiring for this\')]').find()
                if hiring_line:
                ...
                pass
            try:
                job_location_element = WebElementWrapper(
                    job_tile, By.CLASS_NAME, 'job-card-container__metadata-item').find()
                if job_location_element:
                    job_location = job_location_element.text
            except:
                pass
            try:
                apply_method_element = WebElementWrapper(
                    job_tile, By.CLASS_NAME, 'job-card-container__apply-method').find()
                if apply_method_element:
                ...
                try:
                    ...
                    if job_el:
                        ...
                        try:
                            ...
                            print(
                                "Could not write the job to the file! No special characters in the job title/company are allowed!")
                            traceback.print_exc()
                except StaleElementReferenceException:
                    # Handle StaleElementReferenceException by re-locating the element
                    print(
                        "StaleElementReferenceException occurred. Re-locating the element.")
                    job_el = WebElementWrapper(
                        job_tile, By.CLASS_NAME, 'job-card-list__title').find()
                    if job_el:
                        job_el.click()
        ...
        if len(frm_el) > 0:
            for el in frm_el:
                ...
                try:
                    ...
                    elif 'hybri' in radio_text:
                        answer = self.get_answer('hybrid')
                    elif 'days' in radio_text:
                        answer = self.get_answer('hybrid')
                    ...
                    elif 'UK' in radio_text or 'Europe' in radio_text or 'based' in radio_text:
                        answer = self.get_answer('based')
                    # elif 'drug test' in radio_text:
                    #     answer = self.get_answer('drugTest')
                ...
                try:
                    ...
                    if 'proficiency' in question_text:
                        ...
                        answer = self.get_answer('citizenship')
                        ...
                        self.select_dropdown(dropdown_field, choice)
                    elif 'sexual orientation' in question_text:
                        answer = self.get_answer('sexualOrientation')
